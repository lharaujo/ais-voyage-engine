...
def transform_coords_polars(df: pl.DataFrame) -> pl.DataFrame:
    """
    Parses UN/LOCODE DDMM[N/S] DDDMM[E/W] into decimal degrees via regex.
    ...
    return (
        ...
        .with_columns(
            lat=(pl.col("lat_deg") + (pl.col("lat_min") / 60))
            * pl.when(pl.col("lat_dir") == "S").then(-1.0).otherwise(1.0),
            lon=(pl.col("lon_deg") + (pl.col("lon_min") / 60))
            * pl.when(pl.col("lon_dir") == "W").then(-1.0).otherwise(1.0),
        )
...
def process_daily_ais(year, month, day):
    ...
    with tempfile.NamedTemporaryFile(suffix=".csv") as tmp:
        ...
        ais = (
            pl.scan_csv(tmp.name, ignore_errors=True).filter(pl.col("imo").is_not_null()).collect()
        )
